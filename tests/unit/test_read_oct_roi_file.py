...
def test_read_oct_roi_file():
    file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fixtures/test_read_oct_roi_fileROI.txt')
    im_shape = [5, 10, 10]
    out = read_oct_roi_file(file_path, im_shape)
    tifffile.imwrite(file_path[:-4] + '-out.tif', out)
    np.testing.assert_array_equal(out[1, 2, 1:4], [11, 11, 8])
